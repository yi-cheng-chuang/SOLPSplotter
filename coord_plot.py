...
class SOLPSplot:
    ...
    def calcPsiVals(self, sname=None, dsa = None, b2mn = None, geo = None, verbose=True, shift=0):
        """
        ...
        f.y = plot against y
        """
        newbase, tbase, drt, Attempt, i = cs.mast_coord_dir(sname)
    
        try:
            if dsa is None:
                dsa = cs.read_dsa(newbase + '/dsa')
            if geo is None:
                geo = cs.read_b2fgmtry(tbase + '/baserun/b2fgmtry')
            if b2mn is None:
                b2mn = cs.scrape_b2mn(newbase + '/b2mn.dat')                
