...
class equilibrium(object):
    ...
    def load_geqdsk(self,gfile,with_bfield=True,verbose=False,rev_Bt=False, rev_psi=False):
        """
    ...
    def load_MAST(self,shot,time,with_bfield=True,verbose=False):
        ...
        if not self._loaded:
            ...
            for attempt in np.arange(n_attempts)+1:
                t_ellapsed = (time_lib.time()-t0)/60
                try:
                    ...
                    logger.exception('Idam error on attempt {} after {:0.1f} min ({:0.0f} s wait) loading magnetics '
                                     'for shot: {}, time: {}'.format(attempt, t_ellapsed, t_sleep,
                                                                     shot, time))
                    if (attempt == n_attempts) or (t_ellapsed > 120):  # 2 hours
                        # Don't sleep longer than 25 min
                        message = ('Abandoned loading magnetics on attempt {} after {:0.1f} min for '
                                  'shot: {}, t: {}').format(attempt, t_ellapsed, shot, time)
                        logger.info(message)
                        print(message)
                        raise e
                    elif attempt > n_attempts * 0.3:
                        t_sleep *= 1.4
                    time_lib.sleep(t_sleep)
                else:
                    if attempt > 1:
                        message = ('Succeeded loading magnetics on attempt {} after {:0.1f} min for '
                                  'shot: {}, time: {}').format(attempt, t_ellapsed, shot, time)
                        logger.info(message)
                        print(message)
                    break
    ...
    def __calc_psi_deriv(self,method='CD'):
        ...
        if method == 'CD':  #2nd order central differencing on an interpolated grid
...
if __name__=='__main__':
    #Load a MAST shot 
    eq = equilibrium(gfile='g1160718025.01209_974')

    fs = eq.get_fluxsurface(1)
    pass
