...
class midplane_radial:
    ...
    def calc_midplane_profile_method(self, itername, data_struc):
        ...
        if self.DF.data_size == 'small':
            ...
            Ti_J = b2fstate['ti'][1:nx+1, 1:ny+1].transpose()
        ...
        if self.DF.data_size == 'small':
            ...
            if withshift:
                ...
                ap = self.data['midplane_calc'][itername]['average_pair']
                pair = (ap[0]-1, ap[1]-1)
        ...
        mid_ne_pro = np.multiply(
            ne_pro[:, pair[0]], weight) + np.multiply(ne_pro[:, pair[1]], weight_B)
        mid_te_pro = np.multiply(
            te_pro[:, pair[0]], weight) + np.multiply(te_pro[:, pair[1]], weight_B)
        mid_ti_pro = np.multiply(
            ti_pro[:, pair[0]], weight) + np.multiply(ti_pro[:, pair[1]], weight_B)
        mid_neu_pro = np.multiply(
            neu_pro[:, pair[0]], weight) + np.multiply(neu_pro[:, pair[1]], weight_B)
        mid_S_pro = np.multiply(sx[pair[0], :], weight) + \
            np.multiply(sx[pair[1], :], weight_B)

        midplane_profile_dic = {'psiN': psi_coord, 'mid_ne': mid_ne_pro, 'mid_te': mid_te_pro,
                                'mid_nd': mid_neu_pro, 'mid_S': mid_S_pro, 'mid_ti': mid_ti_pro}
    ...
    def oneDscan_midplane(self, iterlist, dat_dic, dat_struc):
        ...
        if series_compare == True:
            ...
            for aa in iterlist:
                ...
                midprofiles = self.calc_midplane_profile_method(
                    itername=aa, data_struc=dat_struc)
    ...
    def twoDscan_midplane(self, iterlist, iterlist_a, iterlist_b, dat_dic, dat_struc):
        ...
        for tp in iterlist:
            ...
            midprofiles = self.calc_midplane_profile_method(
                itername=tp, data_struc=dat_struc)
    ...
    def calc_midplane_profile(self):
        ...
        if withshift == False and withseries == False:
            ...
            midprofile_dic = self.calc_midplane_profile_method(
            ...
            if self.DF.series_flag == 'twin_scan':
                ...
                mid_dic = {}
                midprofile_dic = self.oneDscan_midplane(
                    iterlist=scan, dat_dic=mid_dic, dat_struc=dat_struc)
