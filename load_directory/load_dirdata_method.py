...
class load_dir_method:
    ...
    def mastu_base_dir(self):
        ...
        if self.DF.DEV == 'cross_machine':
            ...
            g_path_a = glob.glob('{}/*'.format(gbase))
            print(g_path_a)
            g_path = glob.glob('{}/*'.format(gbase))[0]
            gdir = Path(g_path).as_posix()
        ...
        if self.DF.DEV == 'cross_machine':
            ...
            newbase = '{}/{}/{}/{}/{}'.format(basedrt, self.DF.DEV,
                                              nd['Shot'], series, filename)
            tbase = '{}/{}/{}/{}'.format(basedrt,
                                         self.DF.DEV, nd['Shot'], series)
    ...
    def mast_fluxstd_dir(self):
        ...
        attempt = str(self.gam.mast_fluxstd_atp_number(
            newbase, usage='load_dir')[0])
        ...
        for aa in a_shift:
            for s in shift_list:
                if aa == s:
                    ...
                    att_dic[aa] = str(self.gam.s_number(
                        adir['fixed']['Output'])[0])
    ...
    def mast_series_dir(self):
        ...
        if self.DF.series_flag == 'ionCX_scan':
            mcds = self.di.mast_comp_dir_ionCXscan()
        ...
        for i in newbase:
            if self.DF.series_flag == 'ionCX_scan':
                ...
                attempt_dic[self.gam.s_number(i)[0]] = self.gam.s_number(i)[0]
                new_dic[self.gam.s_number(i)[0]] = i
            elif self.DF.series_flag == 'eireneN':
                attempt_dic[self.gam.s_number(i)[0][1]] = self.gam.s_number(i)[
                    0][0]
                new_dic[self.gam.s_number(i)[0][1]] = i
            elif self.DF.series_flag == 'change_temp':
                attempt_dic[self.gam.s_number(i)[0][0]] = self.gam.s_number(i)[
                    0][1]
    ...
    def series_terminal_dir(self, dir_comp_dic):
        ...
        for i in newbase:
            if self.DF.series_flag == 'terminal_test':
                attempt_dic[self.gam.s_number(i, self.DF.series_flag)[
                    0][0]] = self.gam.s_number(i, self.DF.series_flag)[0][1]
                new_dic[self.gam.s_number(i, self.DF.series_flag)[0][0]] = i

            elif self.DF.series_flag == 'twin_scan':
                attempt_dic[self.gam.atp_number(i, self.DF.series_flag)[
                    0]] = self.gam.atp_number(i, self.DF.series_flag)[1]
