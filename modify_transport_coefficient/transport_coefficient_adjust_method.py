...
from load_simulation_data.load_B2_data_method import read_b2fstate


class transcoe_method:
    ...
    def load_transcoefile_method(self, file ='b2.transport.inputfile', plot=False):
    ...
    def Generate_transcoefile_method(self, trans_pts, CoeffID = 1, SpeciesID = 1, M=[1]):
        '''
    ...
    def Write_transcoefile_method(self, file='b2.transport.inputfile', points={},M_1 = True, M=[1]):
        inputfile={}
        if points:
            for k in points.keys():
                inputfile[k] = self.Generate_transcoefile_method(points[k].T,CoeffID=int(k),M=M)
        else:
            points = self.load_transcoefile_method(file)
            for k in points.keys():
                inputfile[k] = self.Generate_transcoefile_method(points[k].T,CoeffID=int(k),M=M)
        if M_1 == True:
            for MM in M:
    ...
    def read_transport_files(self, fileloc, dsa=None, geo=None, state=None, force_read_inpufile=False):
        ...
        if dsa is None:
            dsa = self.lcm.read_dsa('dsa')
        
        if geo is None:
            geo = self.lcm.read_b2fgmtry('../baserun/b2fgmtry')
        ...
        if not read_inputfile:
            b2mn = self.lcm.scrape_b2mn("b2mn.dat")
