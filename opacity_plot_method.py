            ...
            for aa in iter_list:
                if i == 'electron_pedestal_density':
                    plt.scatter(result_dic['electron_pedestal_density'][aa], result_dic['efold_length'][aa], 
                             color= color_dic[aa], label= '{} {}'.format(aa, 'efold_length'))
                    plt.scatter(result_dic['electron_pedestal_density'][aa], result_dic['pedestal_width'][aa], 
                             color= color_dic[aa], label= '{} {}'.format(aa, 'pedestal_width'))
                    plt.xlabel('electron_pedestal_density: $n_{ped}$ (m$^{-3}$)')
                    # plt.title('{} and {} verses {} from {:.2f} to {:.2f}'.format('efold length', 
                    #                                          'pedestal width', 'electron pedestal density', pol_loc[aa][0], pol_loc[aa][-1]))
                    ...
                    plt.plot(pol_loc[aa], result_dic[i][aa], 'o--', color= color_dic[aa])
                    ...
                    plt.xlabel('poloidal angle')
                    # plt.legend()
            if max(pol_loc[aa]) > 90 and i != 'electron_pedestal_density':
                plt.axvline(x= 90, color='black',lw=3, ls='--')
            else:
                pass
            # plt.ylabel('{}'.format(unit_dic[i]))
...
def opacity_radial_method_single(result_dic, SEP, x_choice, x_coord, Nd, Ne, Te, 
    ...
    plt.plot(x,y, color='orange', lw=3, label= 'Neutral penetration length : $\lambda_{n_D}$')
    ...
    plt.plot(xd, yd, color='black', lw=3, label= 'Pedestal width : $\Delta n_e$')
...
def opacity_radial_method_multi(x_choice, result_dic, SEP, iter_list, 
    ...
    if withshift == False and withseries == True:
        ...
        if series_flag == 'change_den':
            ...
            for l in iter_list:
                if x_choice == 'psiN':
                    plt.plot(psi_dic[l], Ne_dic[l],'o-', label= 'solps electron density {}'.format(density_dic[l]))
                    # plt.plot(psi_dic[j], tanh_ne_fit_dic[j], color='r',lw= 3, label= 'exponential fit')
                if x_choice == 'psiN':
                    plt.xlabel('psiN')
                # plt.ylabel(P['Ne'])
