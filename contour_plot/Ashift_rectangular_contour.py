# -*- coding: utf-8 -*-
"""
Created on Thu Feb 19 20:50:18 2026
...
class Ashift_Rectangular_contour:
    ...
    def Ashift_rectangular_contourplot(self, plot_name, norm_type, label_type, pol_loc_list, sted_index_list):
        ...
        if withshift == True and withseries == False:
            ...
            for aa in self.data['dircomp']['Attempt'].keys():
                ...
                psi_map = np.transpose(self.data['psi']['psival'][aa])[
                    1:nx+1, 1:ny+1]

                ang_label = self.data['angle']['angle_list']['org']
                ...
                nd_std_dat = self.data['ft44']['org']['dab2'][:, :, 0]
                nm_std_dat = self.data['ft44']['org']['dmb2'][:, :, 0]
                source_std = self.data['b2wdat']['org']['b2npc_sna'][0][1:nx+1, 1:ny+1]
                vol_std = self.data['b2wdat']['org']['vol'][1:nx+1, 1:ny+1]
                ...
                if label_type == 'angle':
                    ...
                    axs.axvline(x=0, color='brown', lw=3, ls='--', label='LFS')
                    axs.axvline(x=180, color='gray',
                                lw=3, ls='--', label='HFS')

                    axs.axhline(y=1, color='red', lw=3,
                                ls='--', label='separatrix')
                    # axs.legend(loc='best')
