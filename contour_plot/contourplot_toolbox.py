...
class contour_plot_method_collect:
    ...
    def contour_plot(self, plot_2dval, R_coord, Z_coord, quantity):
        ...
        plt.figure()
        plt.contourf(R_coord, Z_coord, plot_2dval,
    ...
    def paper_contour(self, plot_2dval, R_coord, Z_coord, quantity, itername,
        ...
        if log_bar:
            ...
            axs.contourf(R_coord, Z_coord, plot_2dval,
    ...
    def eirene_contourplot_method(self, simu_direc, data, itername, axs, datname, norm_type):
        ...
        Nodes = np.fromfile('{}/fort.33'.format(simu_direc),
                            sep=' ')  # Alternatively use fort.33
        NN = int(Nodes[0])
        XNodes = Nodes[1:NN+1]
        YNodes = Nodes[NN+1:]
        ...
        axs.set_title(
            '{0} $\Gamma_r ={1:.3f}$*$10^{{20}}$ 1/s, $q_r = {2:.3f}*10^5$ W'.format(datname, ad, ap))
    ...
    def twcontourp(self, plot_2dval, R_coord, Z_coord, quantity, axs, norm_type, lv, vmin, vmax):
        CMAP = 'jet'
        ...
        if norm_type == 'masklog':
            ...
            normalized_data = (
                plot_2dval - np.mean(plot_2dval)) / np.std(plot_2dval)
        ...
        elif norm_type == 'naturalset':

            NORM = colors.Normalize(vmin, vmax)
            levels = np.linspace(vmin, vmax, lv)

        elif norm_type == 'fixlognorm':
            ...
            levels = np.logspace(logmin, logmax, lv)
            NORM = colors.LogNorm(vmin, vmax)

        if norm_type == 'fixlognorm':
            axs.contourf(R_coord, Z_coord, plot_2dval,
