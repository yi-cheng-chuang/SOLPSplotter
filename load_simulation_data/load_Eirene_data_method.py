...
#    Do consistency check on the data
    if (ver >= 20160829):
        ...
        while fieldname not in line:
            line = fid.readline().rstrip()
            if len(line) == 0:
                print('read_ft44_field: EOF reached without finding '+str(fieldname))
        # Consistency check: number of elements specified in the file should equal
        # prod(dims)
        for i in range(len(line.split())):
            if is_number(line.split()[i]):
                numin = int(line.split()[i])
    ...
    while (len(fieldVal) != numin):
        line = fid.readline().rstrip()
        if ('wld' in fieldname) and len(fieldVal) >= numin-1:
            break
        for i in range(len(line.split())):
            if ('wlpump' in fieldname):
                if not is_number(line.split()[i]):
                    continue
            if (intField):
                fieldVal.append(int(line.split()[i]))
            else:
                fieldVal.append(float(line.split()[i]))
    fieldVal = np.array(fieldVal)

    if (np.size(dims) > 1 and 'wld' not in fieldname):
        fieldVal = fieldVal.reshape(dims, order='F').copy()
    print(dims)
    if len(fieldVal) != numin:
        print(
            f" Mismatch for {fieldname}: expected {numin}, got {len(fieldVal)}")

    if (np.size(dims) > 1 and 'wld' in fieldname):
        if len(fieldVal) < np.prod(dims):
            print(
                f"Warning: field {fieldname} missing {np.prod(dims) - len(fieldVal)} values, padding with zeros.")
            fieldVal = np.pad(fieldVal, (0, np.prod(
                dims) - len(fieldVal)), mode='constant', constant_values=0)
        fieldVal = fieldVal.reshape(dims).copy()
...
def read_ft44(fileName):
    ...
    fid = open(fileName)
    if (fid == -1):
        print("read_ft44: can't open file")
...
# Read basic data, there is more, I might grab it if I find out I need it
    class ft44Results:
        def __init__(self):
            ...
            # self.wall_geometry = read_ft44_field(
            #     fid, ver, 'wall_geometry', [620])
            self.wall_geometry = read_ft44_field(
                fid, ver, 'wall_geometry', [4, 46])
            ...
            for i in range(1, int(istra)+1):
                if i < 10:
                    wldnek.append(read_ft44_field(
                        fid, ver, 'wldnek(  '+str(i)+')', [-1]))
                    wldnep.append(read_ft44_field(
                        fid, ver, 'wldnep(  '+str(i)+')', [-1]))
                    wldna.append(read_ft44_field(
                        fid, ver, 'wldna(  '+str(i)+')', [natm, -1]))
                    ewlda.append(read_ft44_field(
                        fid, ver, 'ewlda(  '+str(i)+')', [natm, -1]))
                    wldnm.append(read_ft44_field(
                        fid, ver, 'wldnm(  '+str(i)+')', [nmol, -1]))
                    ewldm.append(read_ft44_field(
                        fid, ver, 'ewldm(  '+str(i)+')', [nmol, -1]))
                    wldra.append(read_ft44_field(
                        fid, ver, 'wldra(  '+str(i)+')', [natm, -1]))
                    wldrm.append(read_ft44_field(
                        fid, ver, 'wldrm(  '+str(i)+')', [nmol, -1]))
                else:
                    wldnek.append(read_ft44_field(
                        fid, ver, 'wldnek( '+str(i)+')', [-1]))
                    wldnep.append(read_ft44_field(
            ...
            for i in range(1, int(istra)+1):
                if i < 10:
                    wldpeb.append(read_ft44_field(
                        fid, ver, 'wldpeb(  '+str(i)+')', [-1]))
                else:
                    wldpeb.append(read_ft44_field(
                        fid, ver, 'wldpeb( '+str(i)+')', [-1]))
...
#            self.wldspt6   = read_ft44_field(fid,ver,'wldspt(  6)',[natm,-1])
 ...
 #           self.wlpumpM   = read_ft44_field(fid,ver,'wlpump(M)',[nmol,88])
            self.eneutrad = read_ft44_field(
                fid, ver, 'eneutrad', [nx, ny, natm])
...
def read_ft46(fileName):
    ...
    class ft46Results:
        def __init__(self):
            self.pdena = read_ft44_field(
                fid, ver, 'pdena', [ntri, natm])*1e6  # m^{-3}
            self.pdenm = read_ft44_field(fid, ver, 'pdenm', [ntri, nmol])*1e6
            self.pdeni = read_ft44_field(fid, ver, 'pdeni', [ntri, nion])*1e6

            self.edena = read_ft44_field(
                fid, ver, 'edena', [ntri, natm])*1e6*eV  # J m^{-3}
            self.edenm = read_ft44_field(
                fid, ver, 'edenm', [ntri, nmol])*1e6*eV
            self.edeni = read_ft44_field(
                fid, ver, 'edeni', [ntri, nion])*1e6*eV

            self.vxdena = read_ft44_field(
                fid, ver, 'vxdena', [ntri, natm])*1e1  # kg s^{-1} m^{-2}
            self.vxdenm = read_ft44_field(fid, ver, 'vxdenm', [ntri, nmol])*1e1
            self.vxdeni = read_ft44_field(fid, ver, 'vxdeni', [ntri, nion])*1e1

            self.vydena = read_ft44_field(
                fid, ver, 'vydena', [ntri, natm])*1e1  # kg s^{-1} m^{-2}
            self.vydenm = read_ft44_field(fid, ver, 'vydenm', [ntri, nmol])*1e1
            self.vydeni = read_ft44_field(fid, ver, 'vydeni', [ntri, nion])*1e1

            self.vzdena = read_ft44_field(
                fid, ver, 'vzdena', [ntri, natm])*1e1  # kg s^{-1} m^{-2}
