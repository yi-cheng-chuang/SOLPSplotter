...
class radial_plot_opacity:
    ...
    def opacity_radial_method(self, result_dic, SEP, x_coord, Nd, Ne, Te,
        ...
        if log_flag:
            ...
            pass
        plt.plot(x_coord, Nd, '-', color='green',
                 label='solps neutral density')
        # plt.plot(psi_RGI, Nd,'-', color = 'b', label= 'RGI_solps neutral density')
        plt.plot(xcoord_cut, exp_an_fit, color='r',
                 lw=5, ls='-', label='exponential fit')
        plt.axvline(x=max(xcoord_cut), color='orange', lw=3)
        plt.plot(x, y, color='orange', lw=3, ls='-',
        ...
        plt.plot(x_coord, tanh_ne_fit, ls='-',
                 color='r', lw=3, label='tanh fit')
        plt.plot(xd, yd, color='black', ls='-', lw=3,
        ...
        if log_flag:
            ...
            pass
        plt.plot(x_coord, Te, ls='-', color='b',
                 label='solps electron tempurature')
        plt.plot(x_coord, tanh_te_fit, ls='-',
                 color='r', lw=3, label='tanh fit')
        plt.plot(xt, yt, color='black', ls='-', lw=3,
                 label='temperature pedestal width [m]: $\Delta n_e$')
    ...
    def Opacity_study_radial_plot(self, pol_loc):
        ...
        if withshift == False and withseries == False:
            ...
            if dat_size == 'small':
                ...
                psi = self.data['psi']['psi_{}_val'.format(
                    pol_loc[0])][:, 1][1:ny+1]
            ...
            for aa in self.data['dircomp']['multi_shift']:
                ...
                if dat_size == 'small':
                    ...
                    psi = self.data['psi']['psival'][aa][1:ny +
                                                         1, int(pol_loc[0])]
